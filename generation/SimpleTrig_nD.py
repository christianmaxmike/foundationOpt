...
def chris_fnc_random(x, min_num_components=1, max_num_components=8) -> np.ndarray:
    ...
    for batch_idx, batch in enumerate(range(x.shape[0])):
        ...
        for _ in range(num_components):
            amplitude = np.random.uniform(0.05, 1.0) 
...
def obj(params : pd.DataFrame, fnc_dict) -> np.ndarray:
    ...
    for i in range(fnc_dict['num_components']):
        ...
        for dim_idx , dim in enumerate(range(len(fnc_dict['frequencies'][0]))):
            amplitude = fnc_dict['amplitudes'][i][dim_idx]  # Amplitude between 0.5 and 2.0
...
def gen_trigonometric_fncs(batch_size, dims, min_x_value, max_x_value, sequence_length, min_num_components, max_num_components):
    ...
    for i in range(batch_size):
        ...
        for _ in range(num_components):
            ...
            amplitudes.append(comp_amplitudes)
...
def generate(*kwargs):
    ...
    sequence_length = kwargs.sequenceLength  # 100
    min_num_components = kwargs.minComponents  # 2
    max_num_components = kwargs.maxComponents  # 8
    min_x_value = kwargs.minX # 0
    max_x_value = kwargs.maxX # 1 # 100 # 4 * np.pi
    min_dim = kwargs.minDim # 2
    max_dim = kwargs.maxDim # 3
    ...
    fnc_lists = []
    for i in range(batch_size):
        print(f"Processing batch fnc: {i}")
    ...
    for i in range(len(hebo_total_xs)):
        ...
        padded_arr = np.pad(a, ((0, 0), (0, (max_dim-1) - a.shape[1])), mode='constant')
        array_l.append(padded_arr)
    
    # Convert data to numpy array in the right format
    x_batch = np.stack(array_l)
    y_batch = np.stack(hebo_total_ys).squeeze() # [0]

    # save batch file
    save_batch(x_batch, y_batch, fnc_lists, f"data_{args.id}.npz", f"models_{args.id}.dill")
